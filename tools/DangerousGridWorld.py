...
class GridWorld(gym.Env):
    ...
    def __init__(self, deterministic=False):
        ...
        for state in range(self.state_number):
            ...
            if new_state == None or new_state in self.walls:
                self.available_action[state][0] = state
            else:
                self.available_action[state][0] = self.pos_to_state(x - 1, y)

            # Check Right
            new_state = self.pos_to_state(x + 1, y)
            if new_state == None or new_state in self.walls:
                self.available_action[state][1] = state
            else:
                self.available_action[state][1] = self.pos_to_state(x + 1, y)

            # Check Up
            new_state = self.pos_to_state(x, y - 1)
            if new_state == None or new_state in self.walls:
                self.available_action[state][2] = state
            else:
                self.available_action[state][2] = self.pos_to_state(x, y - 1)

            # Check Down
            new_state = self.pos_to_state(x, y + 1)
            if new_state == None or new_state in self.walls:
                self.available_action[state][3] = state
            else:
                self.available_action[state][3] = self.pos_to_state(x, y + 1)
    ...
    def render_policy(self, policy):
        ...
        for i in range(self.state_number):
            ...
            if i in self.walls:
                ...
                print(f" {self.actions[policy[i]]} ", end=" ")
            if (i + 1) % self.map_size == 0: print()
